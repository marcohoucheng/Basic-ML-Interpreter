...
let rec eval_expr (env : value env) (e : expr) : value =
    ...
    | LetRec (f, _, e1, e2) -> 
        ...
        // Similar to App above...
        // eval_expr ((x, v2) :: env1) e
        // eval_expr ((x, v2) :: (f, v1) :: env1) e
        // eval_expr ((x, v1) :: env) e2
        eval_expr ((f, env_RC) :: env) e2
    ...
    | UnOp ("+", e) -> unop_p (+) env e
    | UnOp ("-", e) -> unop_n (-) env e
    // Why do we need two operators?
    // Why does UnOp can't be one?
    // Exponential?
...
and binop_bool op_int op_float env e1 e2 = //Problem: Type does not support float
    let v1 = eval_expr env e1
    let v2 = eval_expr env e2
    match v1, v2 with
